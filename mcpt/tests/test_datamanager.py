...
class TestDataManager(TestBase):
    def __init__(self, data_dir="./data/", name="TestDataManager"):
        super().__init__(name)
        self.tokenizer = AutoTokenizer.from_pretrained(
            "sentence-transformers/all-MiniLM-L6-v2"
        )
        self.data_dir=data_dir
        self.dm = DataManager(self.tokenizer, data_dir=data_dir)
    ...
    def test_drop_classes(self):
        dm = DataManager(self.tokenizer, data_dir=self.data_dir, drop_classes=INT2LABEL[4:7])
    ...
    def test_datamanager_dev(self):
        ...
        dm = DataManager(self.tokenizer, data_dir=self.data_dir, use_dev=True, languages_for_head_eval=["ge"])
        contrastive_dataset = dm.get_contrastive_dataset()
        assert (
    ...
    def test_dataset_contrastive_pairs(self):
        dm = DataManager(self.tokenizer, data_dir=self.data_dir, create_contrastive_pairs=True)
        contrastive_dataset = dm.get_contrastive_dataset()
