...
class Trainer:
    ...
    def train_embeddings(self, n_epochs: int, random_seed: int = 42) -> None:
        losses = list()
        ...
        for epoch in range(n_epochs):
            ...
            model_loader = self._get_dataloader(
                    train_dataset,
                    self.hparams['model_loader_type'],
                    self.hparams['mbs'],
                    seed=random_seed)
            self.bert.train()
            ...
            self.log('WCSL', torch.mean(torch.tensor(losses)).numpy())
            self.model_scheduler.step()
            self.check_epoch_and_save_checkpoint(epoch, losses, 'finetuning')

    def train_head(self, n_epochs: int, random_seed: int = 42) -> None:
        ...
        train_loader = self._get_dataloader(train_embeddings, 'random', self.hparams['hbs'], seed=random_seed)
        losses = list()
        for epoch in range(n_epochs):
            ...
            self.log('BCEloss', torch.mean(torch.tensor(losses)).numpy())
        ...
        if self.validate_every_n_epochs != -1:
            ...
            self.log('KNNlogits', knearest_logits.cpu().numpy())

    def train_hybrid(self, n_epochs: int, random_seed: int = 42) -> None:
        losses = list()
        model_loader = self._get_dataloader(
                self.model_dataset,
                self.hparams['model_loader_type'],
        ...
        for epoch in trange(n_epochs, desc="Epoch"):
            ...
            self.log("Hybridloss", torch.mean(torch.tensor(losses)).numpy())
            self.model_scheduler.step()
            if self.validate_every_n_epochs != -1:
                self.bert.eval()
                train_embeddings = self.compute_embeddings(self.head_dataset)
    ...
    def train_joint(self, n_epochs: int, random_seed: int = 42) -> None:
        model_loader = self._get_dataloader(
                self.model_dataset,
                self.hparams['model_loader_type'],
                self.hparams['mbs'],
        ...
        for epoch in trange(n_epochs, desc='Epoch'):
            ...
            self.head_scheduler.step()
            self.log('BCEloss', torch.mean(torch.tensor(losses)).numpy())
            self.log('WCSL', torch.mean(torch.tensor(contrast_losses)).numpy())
            self.bert.eval()
    ...
    def compute_metrics(self, dataset: TensorDataset, on_eval_dataset: bool = True) -> dict:
        ...
        computed_metrics[f'{prefix}predictions'] = predictions.cpu().numpy()
    ...
    def _get_dataloader(self, dataset: datasets.Dataset, loader_type: str, batch_size: int,
